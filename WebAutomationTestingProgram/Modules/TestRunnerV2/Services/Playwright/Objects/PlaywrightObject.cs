...
namespace WebAutomationTestingProgram.Modules.TestRunnerV2.Services.Playwright.Objects
{
    ...
    public class PlaywrightObject
    {
        ...
        {
            Instance = Microsoft.Playwright.Playwright.CreateAsync().GetAwaiter().GetResult();
        ...
        public async Task ProcessRequestAsync(ProcessRequest request)
        {
            ...
            request.SetStatus(State.Queued, $"Waiting for lock on Browser Type: {request.Browser}, Version: {request.BrowserVersion}.");
            await _lockManager.AquireLockAsync((request.Browser, request.BrowserVersion), request.CancelToken);
            request.SetStatus(State.Processing, $"Lock aquired");

            Browser browser;
            try
            {
                if (_activeBrowsers.TryGetValue((request.Browser, request.BrowserVersion), out browser!))
                {
                ...
                else
                {
                    ...
                    browser = await _browserFactory.CreateBrowser(this, request.Browser, request.BrowserVersion);
                    _activeBrowsers.TryAdd((request.Browser, request.BrowserVersion), browser);
            ...
            {
                _lockManager.ReleaseLock((request.Browser, request.BrowserVersion));
            }
            ...
            finally
            {
                ...
                await _lockManager.AquireLockAsync((request.Browser, request.BrowserVersion));
                browser.DecrementRequestCount(request);

                if (browser.GetRequestCount() == 0)
                {
                    await browser.CloseAsync();
                    _activeBrowsers.TryRemove((request.Browser, request.BrowserVersion), out _);
